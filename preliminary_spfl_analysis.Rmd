...
# Load one season but keep all columns
...
# Combine only the columns needed for this plot
...
# Count Home, Away and Draw results for every season
...
# Create a long dataset with home and away goals in one column
...
logit_model <- glm(HomeWin ~ `Percent Capacity` + `Travel Distance for Away Team`,
                   data = spfl,
                   family = "binomial")
...
# Reload the full 2020/21 dataset because yellow-card columns are needed
spfl_2020_full <- read_excel("SPFL 2020-2021.xlsx", sheet = "SPFL 2020-2021")
...
home_yellows <- table(factor(spfl$`Home Yellow cards`, levels = yellow_values))
away_yellows <- table(factor(spfl$`Away Yellow cards`, levels = yellow_values))
...
barplot(
  ...
  legend.text = c("Home", "Away"),
  col = c("steelblue", "orange"),
  main = "Distribution of Yellow Cards for Home and Away Teams for 2025-26",
  xlab = "Number of Yellow Cards",
  ylab = "Number of Matches"
)
```
...
# 2024/25 Key Match Incident data
...
# Binomial test for each team
# Tests whether decisions against differ from 50% of all decisions
